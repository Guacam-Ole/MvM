...
def edit_cmdline():
	...
	input_incorrect = True
	while input_incorrect:
		value = input("    Are you sure to change the file with new line? (y/n): ")
		if value in ["n", "N"]:
			print("    The screen will not work, but OK.")
			input_incorrect = False
			time.sleep(1)
...
def main():
	...
	input_incorrect = True
	while input_incorrect:
		rotation = input("    Input the rotation of the screen (0, 90, 180, 270, 360 or leave empty for default setting 270): ")
		if rotation == "":
			input_incorrect = False
			rotation = 270
